...
definePolygon::slotApplyDefinePolygon ()
{
  ...
  float min_z = 99999999.9;
  int32_t min_feature = 0;
      ...
      if (inside_polygon2 (bfd_polygon.longitude, bfd_polygon.latitude, bfd_record.poly_count, misc->feature[i].longitude,
                           misc->feature[i].latitude))
        {
          if (options->group_features && misc->feature[i].parent_record)
            {
              QMessageBox::warning (this, "pfmView",
...
definePolygon::slotPolygon ()
{
  if (!mod_chart_scale)
    {
      QMessageBox::warning (this, "pfmView", tr ("You must define a chart scale (other than 0) before defining the polygon."));
      return;
    }
      ...
      if (misc->feature_polygon_flag > 0)
        {
          ...
          int32_t num_points = misc->poly_count;
          int32_t bytes = (((5 * (num_points - 1)) + 1) * sizeof (double));

          double *poly_x = (double *) malloc (bytes);
          double *poly_y = (double *) malloc (bytes);
          if (poly_y == NULL)
            {
              perror (__FILE__);
              exit (-1);
