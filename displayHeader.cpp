  ...
  QDialog (parent, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  //  Check to see if we're using "huge" or "large" files.

  char temp[1024];

#ifdef NVWIN3X
    struct _stat   buf;
    _stat (misc->abe_share->open_args[0].bin_path, &buf);
    if (buf.st_mode & _S_IFDIR)
#else
    struct stat    buf;
    stat (misc->abe_share->open_args[0].bin_path, &buf);
    if (buf.st_mode & S_IFDIR)
      ...
      {
        strcpy (temp, misc->abe_share->open_args[0].bin_path);
      }
