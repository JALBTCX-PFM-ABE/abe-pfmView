  ...
  QDialog (parent, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  change = (int16_t *) calloc (count, sizeof (int16_t));
  ...
  int32_t width = qMin (800, deleteTable->columnWidth (0) + deleteTable->columnWidth (1) + 60);
  int32_t height = qMin (600, deleteTable->rowHeight (0) * count + 75);
  resize (width, height);
  ...
  if (num_files)
    {
      ...
      for (int32_t i = 0 ; i < misc->abe_share->open_args[0].head.bin_height ; i++)
        {
          ...
          for (int32_t j = 0; j < misc->abe_share->open_args[0].head.bin_width; j++)
            {
              coord.x = j;
