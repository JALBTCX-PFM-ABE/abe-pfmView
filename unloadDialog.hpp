...
class unloadDialog:public QDialog
{
  ...
  unloadDialog (QWidget *parent = 0, OPTIONS *op = NULL, MISC *mi = NULL, int32_t dlg = -1, int32_t pfm = -1);
  ~unloadDialog ();

  void get_bounds ();
  void startUnload (OPTIONS *op, MISC *mi, int32_t process);


signals:

  void unloadCompleteSignal (int32_t dialog, int32_t error);
