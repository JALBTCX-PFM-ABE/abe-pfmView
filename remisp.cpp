  ...
  for (int32_t pfm = 0 ; pfm < misc->abe_share->pfm_count ; pfm++)
    {
      ...
      if (misc->abe_share->display_pfm[pfm] && misc->average_type[pfm] > 0)
	{
	  ...
	  progText = tr (" Reading PFM data from %1 ").arg (QFileInfo (QString (misc->abe_share->open_args[pfm].list_path)).fileName ().remove (".pfm"));
  ...
  for (int32_t pfm = 0 ; pfm < misc->abe_share->pfm_count ; pfm++)
      ...
      if (misc->abe_share->display_pfm[pfm] && misc->average_type[pfm] > 0)
	{
	  ...
	  //  Initialize the MISP engine.
          ...
          progText = tr (" Loading data into MISP for %1 ").arg (QFileInfo (QString (misc->abe_share->open_args[pfm].list_path)).fileName ().remove (".pfm"));
	  ...
	  for (int32_t i = 0 ; i < out_count ; i++)
	      ...
	      if (!(out_count % inc))
		{
          ...
          progText = tr (" Computing MISP surface for %1 ").arg (QFileInfo (QString (misc->abe_share->open_args[pfm].list_path)).fileName ().remove (".pfm"));
                          ...
                          if (bin_inside_ptr (&misc->abe_share->open_args[pfm].head, xy))
                            {
                              ...
                              if (!(bin.validity & PFM_DATA) && (!land_mask_flag || !(bin.validity & PFM_USER_10)))
                                {
