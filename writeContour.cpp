...
void writeContour (MISC *misc, float z_factor, float z_offset, int32_t count, double *cur_x, double *cur_y)
{
  ...
  static int16_t line_num[MAX_ABE_PFMS];
  ...
  if (first)
    {
      ...
      for (int32_t pfm = 0 ; pfm < MAX_ABE_PFMS ; pfm++) line_num[pfm] = 0;
    }
  ...
  for (int32_t pfm = 0 ; pfm < misc->abe_share->pfm_count ; pfm++)
      ...
      if (misc->contour_in_pfm[pfm] && misc->abe_share->display_pfm[pfm] && misc->average_type[pfm])
	{
	  ...
	  int32_t file_num = -1;
	  int32_t file_count = get_next_list_file_number (misc->pfm_handle[pfm]);
	  ...
	  if (file_num == -1)
	    {
	      ...
	      line_num[pfm] = write_line_file (misc->pfm_handle[pfm], name);
          ...
          else
            {
              ...
              line_num[pfm] = -1;
              int32_t line_count = get_next_line_number (misc->pfm_handle[pfm]);
	  ...
	  max_coord.y = -999999999;
          ...
          double min_cell_size = qMin (misc->ss_cell_size_x[pfm], misc->ss_cell_size_y[pfm]);
                              ...
                              if (misc->function != GRAB_CONTOUR ||
                                                   ...
                                                   depth_record.xyz.x, depth_record.xyz.y))
                                {
                                  ...
                                  if ((status = add_depth_record_index (misc->pfm_handle[pfm], &depth_record)))
                                    {
                                      if (status != ADD_DEPTH_RECORD_OUT_OF_LIMITS_ERROR)
                                        {
                                          fprintf (stderr, "%s %s %s %d - add_depth_record_index - %s\n", misc->progname, __FILE__, __FUNCTION__, __LINE__,
	  ...
	  for (int32_t i = min_coord.y ; i <= max_coord.y ; i++)
	    {
	      coord.y = i;

	      for (int32_t j = min_coord.x ; j <= max_coord.x ; j++)
		{
		  coord.x = j;
