...
void highlight_contour (MISC *misc, OPTIONS *options, double lat, double lon, nvMap *map)
{
  ...
  float save_level = (float) (NINT (bin.avg_filtered_depth));
  ...
  if (misc->abe_share->cint == 0.0)
    {
      ...
      int32_t j = 0;
      for (int32_t i = 0 ; i < misc->abe_share->num_levels ; i++)
	{
	  float diff = fabs (save_level - misc->abe_share->contour_levels[i]);
	  if (diff < mindiff)
	    {
	      diff = mindiff;
	      j = i;
  ...
  //  Normal contour interval...
    ...
    {
      int32_t mult = NINT ((save_level * options->z_factor + options->z_offset) / misc->abe_share->cint);
      misc->draw_contour_level = misc->abe_share->cint * (float) mult;
    }
  ...
  if (options->contour)
      ...
      for (int32_t pfm = misc->abe_share->pfm_count - 1 ; pfm >= 0 ; pfm--)
	{
	  if (misc->abe_share->display_pfm[pfm])
		...
		{
                  fprintf (stderr, "%s %s %s %d - current_record array - %s\n", misc->progname, __FILE__, __FUNCTION__, __LINE__,
                           strerror (errno));
		  exit (-1);
		}
	      ...
	      for (int32_t jj = misc->hatchr_start_y ; jj < misc->hatchr_end_y ; jj++)
		{
		  ...
		  read_bin_row (misc->pfm_handle[pfm], misc->displayed_area_width[pfm], misc->displayed_area_row[pfm] + jj, 
				misc->displayed_area_column[pfm], current_record);
                  ...
                  loadArrays (misc, pfm, current_record, misc->current_row, NULL, 0, misc->current_flags, options->highlight, options->h_count,
                              options->highlight_percent);
