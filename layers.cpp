...
void swap_layers (MISC *misc, int32_t l1, int32_t l2)
{
  ...
  uint8_t display_pfm = misc->abe_share->display_pfm[l1];

  double ss_cell_size_x = misc->ss_cell_size_x[l1];
  double ss_cell_size_y = misc->ss_cell_size_y[l1];
  char attr_format[NUM_ATTR][20];
  int32_t num_bin_attr = misc->abe_share->open_args[l1].head.num_bin_attr;
  for (int32_t i = 0 ; i < num_bin_attr ; i++) strcpy (attr_format[i], misc->attr_format[l1][i]);
  int32_t pfm_handle = misc->pfm_handle[l1];
  int32_t average_type = misc->average_type[l1];
  uint8_t cube_attr_available = misc->cube_attr_available[l1];
  int32_t pfm_alpha = misc->pfm_alpha[l1];
  int32_t last_saved_contour_record = misc->last_saved_contour_record[l1];
  ...
  for (int32_t i = 0 ; i < misc->abe_share->open_args[l2].head.num_bin_attr ; i++)
  ...
  for (int32_t i = 0 ; i < num_bin_attr ; i++) strcpy (misc->attr_format[l2][i], attr_format[i]);
...
void remove_layer (MISC *misc, int32_t l1)
{
  ...
  for (int32_t pfm = l1 ; pfm < misc->abe_share->pfm_count - 1 ; pfm++)
    {
...
void move_layer_to_top (MISC *misc, int32_t l1)
{
  ...
  for (int32_t i = 0 ; i < num_bin_attr ; i++) strcpy (misc->attr_format[0][i], attr_format[i]);
...
void move_layer_to_bottom (MISC *misc, int32_t l1)
{
  ...
  for (int32_t i = 0 ; i < num_bin_attr ; i++) strcpy (misc->attr_format[misc->abe_share->pfm_count - 1][i], attr_format[i]);
