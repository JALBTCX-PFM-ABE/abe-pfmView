...
{
  int32_t             i, j, x, y, z, pixels = 0, fixed_pixels = 12;
  double              wlon, elon, slat, nlat, dep, nx[2], ny[2];
  NV_I32_COORD2       coord;
  uint32_t            val;
  QColor              tmp, tmp_opaque, tmp_info;
          ...
          for (i = 0 ; i < misc->displayed_area_height[pfm] ; i++)
            {
              ...
              double lat = misc->displayed_area[pfm].min_y + ((double) i + 0.5) * misc->abe_share->open_args[pfm].head.y_bin_size_degrees;

              for (j = 0 ; j < misc->displayed_area_width[pfm] ; j++)
                {
                  coord.x = misc->displayed_area_column[pfm] + j;
                  double lon = misc->displayed_area[pfm].min_x + ((double) j + 0.5) * misc->abe_share->open_args[pfm].head.x_bin_size_degrees;
  ...
  if (options->display_feature)
    {
      if (misc->def_feature_poly)
        ...
        {
          if (options->fixed_feature_size)
            {
              pixels = fixed_pixels;
              tmp_opaque = options->feature_color;
              tmp_opaque.setAlpha (255);
            }
          else
            {
              ...
              map->map_to_screen (1, &mapdef.center_x, &mapdef.center_y, &dep, &pix_x[0], &pix_y[0], &pix_z);
              newgp (mapdef.center_y, mapdef.center_x, 0.0, misc->feature_poly_radius * 2, &ny[0], &nx[0]);
              map->map_to_screen (1, &nx[0], &ny[0], &dep, &pix_x[1], &pix_y[1], &pix_z);

              pixels = pix_y[0] - pix_y[1] + 1;
            }
                              ...
                              if (options->feature_poly_color.alpha () < 255)
                                {
                                  ...
                                  map->drawCircle (x, y, NINT ((float) fixed_pixels * 0.5), 0.0, 360.0, LINE_WIDTH, tmp_opaque, Qt::SolidLine, NVFalse);
                                }
                              else
                                {
                              ...
                              if (options->display_children || !misc->feature[i].parent_record)
                                {
                                  ...
                                  if (options->display_feature_info || options->display_feature_poly || highlight)
                                    {
                                      tmp_info = options->feature_info_color;


                                      //  Don't display info for child features.

                                      if (options->display_feature_info && !feature_info.isEmpty () && !misc->feature[i].parent_record)
                                        map->drawText (feature_info, x + 6, y + 3, tmp_info, NVFalse);
                                  ...
                                  if (misc->feature[i].parent_record)
                                    {
                                      if (options->feature_color.alpha () < 255)
                                        {
                                          map->drawCircle (x, y, NINT ((float) fixed_pixels * 0.5), 0.0, 360.0, LINE_WIDTH, tmp_opaque, Qt::SolidLine,
                                                           NVFalse);
                                        }
                                      else
                                        {
                                          map->drawCircle (x, y, NINT ((float) pixels * 0.5), 0.0, 360.0, LINE_WIDTH, tmp, Qt::SolidLine, NVFalse);
                                        }
                                    }
                                  else
                                    {
                                      if (options->feature_color.alpha () < 255)
                                        {
                                          ...
                                          map->drawCircle (x, y, fixed_pixels, 0.0, 360.0, LINE_WIDTH, tmp_opaque, Qt::SolidLine, NVFalse);
                                        }
                                      else
                                        {
                                          map->drawCircle (x, y, pixels, 0.0, 360.0, LINE_WIDTH, featureColor, Qt::SolidLine, NVFalse);
                                        }
                                    }
                                }
                            }
      ...
      if (misc->def_feature_poly)
        {
          ...
          if (misc->poly_count)
            {
              for (int32_t j = 1 ; j < misc->poly_count ; j++)
                {
