  ...
  if (misc->abe_share->pfm_count)
    {
      ...
      if ((pfm_handle = open_existing_pfm_file (&open_args)) < 0)
        {
          QMessageBox::warning (parent, pfmView::tr ("pfmView Open PFM Structure"),
      ...
      if (min_dist > 100000.0)
        {
          QMessageBox::warning (parent, pfmView::tr ("pfmView Open PFM Structure"),
                                pfmView::tr ("The file \n%1\n is more than 100 kilometers away from the currently displayed area.\n\n"
                                             "If you wish to open this file you must close the currently opened PFM(s).").arg
                                (QString (gen_basename (open_args.list_path))));
