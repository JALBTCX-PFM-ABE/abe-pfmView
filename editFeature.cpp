  ...
  QDialog (pt, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  if (new_feature)
    ...
    {
      featureType->setCurrentIndex (bfd_record.feature_type);
    }
...
editFeature::slotChartScale (int value)
{
  ...
  misc->feature_poly_radius = ((options->millimeters / 1000.0) * mod_chart_scale) / 2.0;
}
...
editFeature::slotViewError (QProcess::ProcessError error)
{
  switch (error)
    ...
    case QProcess::WriteError:
      QMessageBox::critical (this, tr ("pfmView image viewer"), tr ("There was a write error to the image viewer process!"));
      break;

    case QProcess::ReadError:
      QMessageBox::critical (this, tr ("pfmView image viewer"), tr ("There was a read error from the image viewer process!"));
      break;

    case QProcess::UnknownError:
      QMessageBox::critical (this, tr ("pfmView image viewer"), tr ("The image viewer process died with an unknown error!"));
      break;
    }
}
              ...
              if ((misc->bfd_handle = binaryFeatureData_open_file (filename, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                  ...
                  if ((misc->bfd_handle = binaryFeatureData_create_file (misc->abe_share->open_args[0].target_path, misc->bfd_header)) < 0)
                    {
                      ...
                      QMessageBox::warning (parent, "pfmView", tr ("Unable to create feature file\nReason: %1").arg (msg));
                      return;
                    }
              ...
              else
                {
                  ...
                  if ((misc->bfd_handle = bfd_check_file (misc, misc->abe_share->open_args[0].target_path, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                    {
                      QString msg = QString (binaryFeatureData_strerror ());
                      QMessageBox::warning (parent, "pfmView", tr ("Unable to open feature file\nReason: %1").arg (msg));
                      return;
                    }
...
editFeature::slotDelete ()
{
  ...
  if (QMessageBox::information (this, "pfmView", 
...
editFeature::slotGroup ()
{
  ...
  float min_z = bfd_record.depth;
  int32_t min_feature = -1;

  for (uint32_t i = 0 ; i < misc->bfd_header.number_of_records ; i++)
    {
      if (inside_polygon2 (bfd_polygon.longitude, bfd_polygon.latitude, bfd_record.poly_count, misc->feature[i].longitude, misc->feature[i].latitude))
        {
          if (misc->feature[i].parent_record)
            {
              QMessageBox::warning (this, "pfmView", tr ("A sub-feature already exists within the polygon.\nPlease un-group the features first."));
              return;
            }

          if (misc->feature[i].child_record)
            {
              QMessageBox::warning (this, "pfmView", tr ("A master feature already exists within the polygon.\nPlease un-group the features first."));
              return;
            }
  ...
  uint8_t shoalest = NVFalse;

  if (min_feature >= 0)
    {
      QMessageBox msgBox (this);
      msgBox.setIcon (QMessageBox::Question);
      msgBox.setText ("pfmView");
  ...
  int32_t poly_count = bfd_record.poly_count;
  int32_t poly_type = bfd_record.poly_type;
  ...
  if (binaryFeatureData_write_record (misc->bfd_handle, bfd_record.record_number, &bfd_record, &bfd_polygon, NULL) < 0)
    {
      QString msg = QString ("Error writing BFD record in file %1, function %2, line %3 - %4\n").arg (__FILE__).arg
        (__FUNCTION__).arg (__LINE__).arg (binaryFeatureData_strerror ());
...
editFeature::slotUngroup ()
{
  ...
  if (!bfd_record.child_record || bfd_record.parent_record)
    {
      QMessageBox::warning (this, "pfmView", tr ("This is not a master feature."));
      return;
    }
