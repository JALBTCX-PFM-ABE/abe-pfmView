...
{
  int32_t                 index_contour, num_points, num_interp, bytes, width, ncc, nrr, digits, cont_index_1,
                          cont_index_2, win_index_1 = 0, win_index_2, label_len_chars, label_x = 0, label_y = 0;
  double                  dcontour_x[CONTOUR_POINTS], dcontour_y[CONTOUR_POINTS], dcontour_z[CONTOUR_POINTS], dx, dy,
  ...
  void writeContour (MISC *misc, float z_factor, float z_offset, int32_t count, double *cur_x,
                     double *cur_y);
  ...
  if (misc->function == GRAB_CONTOUR && misc->poly_count)
    {
      ...
      for (int32_t gpfm = 0 ; gpfm < misc->abe_share->pfm_count ; gpfm++) misc->contour_in_pfm[gpfm] = NVTrue;
      capture = NVTrue;
    }
  ...
  while (getContour (&level, &index_contour, &num_points, contour_x, contour_y))
    {
      ...
      //  bounding polygon (in writeContour).
      ...
      if (capture)
        {
          ...
          //  Check to make sure that at least two cells are within the polygon.  Otherwise there's no point in calling
          //  writeContour.
  ...
  if (capture)
    {
      ...
      for (int32_t gpfm = 0 ; gpfm < misc->abe_share->pfm_count ; gpfm++) misc->contour_in_pfm[gpfm] = NVFalse;
    }
