  ...
  QDialog (parent, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  int32_t width = qMin (800, queueTable->columnWidth (0) + queueTable->columnWidth (1) + 60);
  int32_t height = qMin (600, queueTable->rowHeight (0) * misc->abe_share->delete_file_queue_count + 75);
  resize (width, height);
...
deleteQueue::slotAccept ()
{
  ...
  int32_t             recnum, **pfm_list, *file_count;
  ...
  pfm_list = (int32_t **) calloc (misc->abe_share->pfm_count, sizeof (int32_t *));
  if (pfm_list == NULL)
    {
      fprintf (stderr, "%s %s %s %d - pfm_list - %s\n", misc->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
      exit (-1);
    }

  file_count = (int32_t *) calloc (misc->abe_share->pfm_count, sizeof (int32_t));
  if (file_count == NULL)
    {
      fprintf (stderr, "%s %s %s %d - file_count - %s\n", misc->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
      exit (-1);
    }
  ...
  for (int32_t i = 0 ; i < misc->abe_share->delete_file_queue_count ; i++)
    {
      pfm_list[misc->abe_share->delete_file_queue[i][0]] =
        (int32_t *) realloc (pfm_list[misc->abe_share->delete_file_queue[i][0]],
                              (file_count[misc->abe_share->delete_file_queue[i][0]] + 1) * sizeof (int32_t));
      ...
      if (file_count[pfm])
        {
          ...
          for (int32_t i = 0 ; i < misc->abe_share->open_args[pfm].head.bin_height ; i++)
            {
              ...
              for (int32_t j = 0; j < misc->abe_share->open_args[pfm].head.bin_width; j++)
                {
                  coord.x = j;
                  ...
                  if (!read_depth_array_index (misc->pfm_handle[pfm], coord, &depth, &recnum))
                    {
                      found = NVFalse;
                      for (int32_t m = 0 ; m < recnum ; m++)
                        {
                          for (int16_t k = 0 ; k < file_count[pfm] ; k++)
                            {
